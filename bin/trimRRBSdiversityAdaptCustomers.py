...
# FUNCTION HELPERS
def getEmptyStats() -> Dict[str, Union[int, List[int]]]:
    return {
        "filename": 0,
        "nrBothRE": 0,
        "nrFwdRE": 0,
...
def FastqIterator(fh: Union[str, gzip.GzipFile]) -> Iterator[Tuple[str, str, str]]:
    """return an iterator of Records found in file handle, fh."""

    def readTotitle(
        fh: Union[gzip.GzipFile, "io.TextIOWrapper"], titleChar: str
    ...
    while nextTitleLine is not None:
        ...
        if len(qualTitle.strip()) > 0 and seqTitle != qualTitle:
            ...
            print(qualTitle)
            raise Exception(
                "Error in parsing: @title sequence entry must be immediately followed by corresponding +title quality entry."
            )
        seqLines = preLines
        qualLines = []
        for i in range(
            len(seqLines)
        ):  # Quality characters should be the same length as the sequence
...
def trimOneRecord(
    ...
    fwdSeq: str,
    fwdQual: str,
    revTitle: Union[str, None],
    revSeq: Union[str, None],
    revQual: Union[str, None],
    outFwd: Union[gzip.GzipFile, "io.TextIOWrapper"],
    outRev: Union[gzip.GzipFile, "io.TextIOWrapper", None],
    stats: Dict[str, Union[int, List[int]]],
) -> None:
    global msp1, taq1, both_taq1_msp1
    ...
    if fwdmPos == -1 and fwdnmPos == -1 and cgaPos == -1:
        ...
        if not SAVE_UNDIGESTED:
            return
    elif cgaPos == -1 and revSeq is not None:
        stats["nrFwdRE"] += 1
        if not SAVE_UNDIGESTED:
            return
    elif cgaPos == -1 and revSeq is None:  ##except if it's a single read
        stats["nrFwdRE"] += 1
    ...
    if yggPos == 0:
        ...
        if fwdSeq[0] not in BASE_POSITIONS:
            print(
                f"Hmm... odd letter {fwdSeq[0]} in {fwdTitle} at fwdSeq[0] when yggPos == 1"
            )
        ...
        if fwdSeq[0] not in BASE_POSITIONS:
            print(
                f"Hmm... odd letter {fwdSeq[0]} in {fwdTitle} at fwdSeq[0] when yggPos == 2"
            )
        if fwdSeq[1] not in BASE_POSITIONS:
            print(
                f"Hmm... odd letter {fwdSeq[1]} in {fwdTitle} at fwdSeq[1] when yggPos == 2"
            )
        for i in range(NUM_BASES):
            if fwdSeq[0] == BASE_POSITIONS[i]:
                stats["bdD2base1"][i] += 1
        ...
        if fwdSeq[0] not in BASE_POSITIONS:
            print(
                f"Hmm... odd letter {fwdSeq[0]} in {fwdTitle} at fwdSeq[0] when yggPos == 3"
            )
        if fwdSeq[1] not in BASE_POSITIONS:
            print(
                f"Hmm... odd letter {fwdSeq[1]} in {fwdTitle} at fwdSeq[1] when yggPos == 3"
            )
        if fwdSeq[2] not in BASE_POSITIONS:
            print(
                f"Hmm... odd letter {fwdSeq[2]} in {fwdTitle} at fwdSeq[2] when yggPos == 3"
            )
        for i in range(NUM_BASES):
            if fwdSeq[0] == BASE_POSITIONS[i]:
                stats["bdD3base1"][i] += 1
        ...
        raise Exception(f"Impossible location for YGG: {yggPos} in {fwdSeq}")
    ...
    if yggPos >= 0:  # if YGG position is found
        if revSeq is None:  # if this is a single read, trim only 5 bases from 3' end
            outFwd.write(
                f"@{fwdTitle}\n{fwdSeq[yggPos:-5]}\n+\n{fwdQual[yggPos:-5]}\n"
            )  # trim off the diversity before YGG site and 5 bases from 3' end
        elif (
            revSeq is not None
        ):  # if this is a paired end read, trim 1 extra base from the 3' end, total of 6 bases so that alignment will be better for short fragments.
            outFwd.write(
                f"@{fwdTitle}\n{fwdSeq[yggPos:-6]}\n+\n{fwdQual[yggPos:-6]}\n"
            )  # trim off the diversity before YGG site and 6 bases from the 3' end
    else:  # if no YGG site is found
        if revSeq is None:  # if this is a single read, trim only 5 bases from 3' end
            outFwd.write(f"@{fwdTitle}\n{fwdSeq[:-5]}\n+\n{fwdQual[:-5]}\n")
        elif (
            revSeq is not None
        ):  # if this is a paired end read, trim 1 extra base from the 3' end, total of 6 bases so that alignment will be better for short fragments.
            outFwd.write(f"@{fwdTitle}\n{fwdSeq[:-6]}\n+\n{fwdQual[:-6]}\n")
    ...
    if cgaPos == 0:
        ...
        raise Exception(f"Impossible location for CGA: {cgaPos} in {revSeq}")
    ...
    if cgaPos >= 0:  # if cga position is found
        outRev.write(
            f"@{revTitle}\n{revSeq[cgaPos+2:-6]}\n+\n{revQual[cgaPos+2:-6]}\n"
        )  # trim the cg from the 5' end and 6 bases from the 3' end
...
if len(fwdFiles) == 0:
    print(f"There are no files that fit the fwd pattern '{fwdFilenames}'")
    print()
    print(HELP_STRING)
    sys.exit(1)
...
if len(revFiles) == 0:
    print(
        "A file pattern wasn't provided for read 2. Assuming the run is single ended."
    )
...
for i in range(len(fwdFiles)):
    ...
    if revFilenames and fwdTitle.split()[0] != revTitle.split()[0]:
        ...
        print(f"fwd title: '{fwdTitle.split()[0]}'")
        print(f"rev title: '{revTitle.split()[0]}'")
    ...
    while True:
        ...
        trimOneRecord(
            fwdTitle, fwdSeq, fwdQual, revTitle, revSeq, revQual, outFwd, outRev, stats
        )
    ...
    print(f"\tDone with this pair. there were {recCount} records")
    ...
    if revFilenames:
        ...
        other = stats["nrNoRE"]
    print(
        f"\tFwd:  D0:{stats['nrD0']}  D1:{d1}  D2:{d2}  D3:{d3} other={other} (total={stats['nrD0']+d1+d2+d3+other})"
    )
